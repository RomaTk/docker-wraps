...
function getBuildOptions {
    ...
    while true; do 
        ...
        [ $? -ne 0 ] && throwError 136 "$is_as_abstract"

        if [[ "$is_as_abstract" == "false" ]]; then
            break
        fi
    ...
    for (( i=$len-1; i>=0; i-- )); do
        options=$(getImplementSpecificData "${based_ons[$i]}" "$options" "$file_with_config" "$config_dir" "build-options")
