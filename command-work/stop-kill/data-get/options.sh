...
function getOptions {
    ...
    local wrap_name="$5"
    local start_options="$6"
    local folder_to_get_data="$7"
    ...
    while true; do 
        ...
        [ $? -ne 0 ] && throwError 129 "$wrap"

        if [[ -z "$wrap" ]]; then
            throwError 131 "Was looking for wrap with name \"$wrap_name\""
        fi

        based_on=$(getBasedOn "$wrap")
        [ $? -ne 0 ] && throwError 130 "$based_on"
        ...
        [ $? -ne 0 ] && throwError 132 "$is_precreate"

        is_as_abstract=$(getAsAbstract "$based_on")
        [ $? -ne 0 ] && throwError 144 "$is_as_abstract"
        ...
        [ $? -ne 0 ] && throwError 133 "$wrap_name"

        is_in_array=$(checkIsInArray "$wrap_name" "${based_ons[@]}")
        [ $? -ne 0 ] && throwError 134 "$is_in_array"
    ...
    for (( i=$len-1; i>=0; i-- )); do
        options=$(getImplementSpecificData "${based_ons[$i]}" "$options" "$file_with_config" "$config_dir" "$folder_to_get_data")
        [ $? -ne 0 ] && throwError 136 "$options"
    done
